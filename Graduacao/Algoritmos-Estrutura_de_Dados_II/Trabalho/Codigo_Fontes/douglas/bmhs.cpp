...
void BoyerMooreHorspoolSunday(TipoTexto T, long n, TipoPadrao P, long m, int *comparacoes, int *deslocamentos) {
    ...
    for (j=0; j <= MAXCHAR; j++){ d[j] = m+1; } // Inicia a tabela de deslocamento de todos os caracteres do alfabeto com o tamanho do padrão +1
    for (j=1; j<=m; j++){  d[P[j-1]] = m-j+1; } // Altera o deslocamento dos caracteres que estão contidos no padrão para o deslocamento correto
    i = m;
	(*comparacoes)++;
    while(i <= n) {
